    ...
    *detail = NULL;
    ...
    if (pNewEntry != pActualEntry) {
        ...
        if (pkg_id == id) {            
            ...
            int* addr = (int*) (c + 128 + 4 + sizeof(size_t) + 4);
            unsigned* size = (unsigned*) (c + 128 + 4 + sizeof(size_t) + 4 + 4);
            ALOGI("analysisLog - dvmLoadNativeLib - { file: %s, addr: 0x%x, size: %d (0x%08x) }", name, addr[0], size[0], size[0]);
        }
        //=========================JNI detection code end=========================//
