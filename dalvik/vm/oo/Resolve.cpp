...
void patchClass(const DexFile* pDexFile, const ClassObject* clazz) {
    ...
    for (u4 i = 0; i < num_class_defs; i++) {
        ...
        if (strcmp(class_descriptor, clazz->descriptor) == 0) {
            ...
            if (pClassData->header.directMethodsSize > 0) {
                for (u4 j = 0; j < pClassData->header.directMethodsSize; j++) {
                    DexMethod* direct_method = &(pClassData->directMethods[j]);
                    const DexCode* direct_dexCode = dexGetCode(pDexFile, direct_method);
                    Method* clazz_method = &(clazz->directMethods[j]);
                    const DexCode* methodCode = dvmGetMethodCode(clazz_method);
                    void* destination = (void* )direct_dexCode->insns;
                    const void* source = (const void* ) methodCode->insns;
                    size_t size = 2 * methodCode->insnsSize;
    ...
    char resdexfppath[255] = {0};
    strcpy(resdexfppath, "/data/data/");
    strcat(resdexfppath, pkgname);
    strcat(resdexfppath, "/classes_resolve.dex");
    FILE* dexfp = NULL;
    dexfp = fopen(resdexfppath, "wb");
    ...
    bool fromUnverifiedConstant)
{
    ...
    if (resClass != NULL) {
        ...
        //=========================instrumentation start=========================//
        if (pkg_id == id && dex_class_name[0] != 0 && dex_method_name[0] != 0) {
            if (strstr(resClass->descriptor, dex_class_name)) {
                for (int i = 0; i < resClass->directMethodCount; i++) {
                    
                    Method* direct_method = &(resClass->directMethods[i]);
                    const DexCode* direct_methodCode = dvmGetMethodCode(direct_method);
                    ALOGI("analysisLog - dexDetection - dvmDexGetResolvedClass - methodName = %s, (from class: %s), insns = %p, insnsSize = %d", direct_method->name, direct_method->clazz->descriptor, direct_methodCode->insns, direct_methodCode->insnsSize);

                    if (strcmp(direct_method->name, dex_method_name) == 0) {
                        char d_codefppath[255] = {0};
                        strcpy(d_codefppath, "/data/data/");
                        strcat(d_codefppath, pkgname);
                        strcat(d_codefppath, "/");
                        strcat(d_codefppath, direct_method->name);
                        strcat(d_codefppath, "_r.dexcode");
                        FILE* d_codefp = NULL;
                        d_codefp = fopen(d_codefppath, "wb");
                        fwrite(direct_methodCode->insns, 2, direct_methodCode->insnsSize, d_codefp);
                        fclose(d_codefp);
                    }
                }

                for (int j = 0; j < resClass->virtualMethodCount; j++) {
                    
                    Method* virtual_method = &(resClass->virtualMethods[j]);
                    const DexCode* virtual_methodCode = dvmGetMethodCode(virtual_method);
                    ALOGI("analysisLog - dexDetection - dvmDexGetResolvedClass - methodName = %s, (from class: %s), insns = %p, insnsSize = %d", virtual_method->name, virtual_method->clazz->descriptor, virtual_methodCode->insns, virtual_methodCode->insnsSize);

                    if (strcmp(virtual_method->name, dex_method_name) == 0) {
                        char v_codefppath[255] = {0};
                        strcpy(v_codefppath, "/data/data/");
                        strcat(v_codefppath, pkgname);
                        strcat(v_codefppath, "/");
                        strcat(v_codefppath, virtual_method->name);
                        strcat(v_codefppath, "_r.dexcode");
                        FILE* v_codefp = NULL;
                        v_codefp = fopen(v_codefppath, "wb");
                        fwrite(virtual_methodCode->insns, 2, virtual_methodCode->insnsSize, v_codefp);
                        fclose(v_codefp);
                    }
                }
            } 
    ...
    if (pkg_id == id && dex_class_name[0] != 0 && dex_method_name[0] != 0) {        
        if (strstr(resClass->descriptor, dex_class_name)) {
            for (int i = 0; i < resClass->directMethodCount; i++) {
                ...
                Method* direct_method = &(resClass->directMethods[i]);
                const DexCode* direct_methodCode = dvmGetMethodCode(direct_method);
                ALOGI("analysisLog - dexDetection - dvmFindClass - methodName = %s, (from class: %s), insns = %p, insnsSize = %d", direct_method->name, direct_method->clazz->descriptor, direct_methodCode->insns, direct_methodCode->insnsSize);
            ...
            for (int j = 0; j < resClass->virtualMethodCount; j++) {
                ...
                Method* virtual_method = &(resClass->virtualMethods[j]);
                const DexCode* virtual_methodCode = dvmGetMethodCode(virtual_method);
                ALOGI("analysisLog - dexDetection - dvmFindClass - methodName = %s, (from class: %s), insns = %p, insnsSize = %d", virtual_method->name, virtual_method->clazz->descriptor, virtual_methodCode->insns, virtual_methodCode->insnsSize);
    ...
    MethodType methodType)
{
    ...
    if (pkg_id == (int) getuid() && strcmp(enable_java_method_trace, "true") == 0) {
        int comIncluded = strncmp(resClass->descriptor, "Lcom", 4);
        if (comIncluded == 0) {
            char* signature = dexProtoCopyMethodDescriptor(&resMethod->prototype);
            ALOGI("analysisLog - Java trace - dvmResolveMethod - { method: %s, type: %s, signature: %s, in class: %s }", resMethod->name, dvmMethodTypeStr(methodType), signature, resClass->descriptor);
        }
    }
    //=========================detection end=========================//
...
Method* dvmResolveInterfaceMethod(const ClassObject* referrer, u4 methodIdx)
{
    ...
    if (pkg_id == (int) getuid() && strcmp(enable_java_method_trace, "true") == 0) {
        ...
        if (comIncluded == 0) {
            char* signature = dexProtoCopyMethodDescriptor(&resMethod->prototype);
            ALOGI("analysisLog - Java trace - dvmResolveInterfaceMethod - { method: %s, signature: %s, in class: %s }", resMethod->name, signature, resClass->descriptor);
        }
    }
    //=========================detection end=========================//
