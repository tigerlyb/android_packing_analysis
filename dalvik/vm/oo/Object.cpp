...
/*
 * JNI trace debugging - Dump some information about an object.
 */
void JNIDumpObject(const Object* obj)
{
    ClassObject* clazz;
    int i;

    if (obj == NULL || obj->clazz == NULL) {
        ALOGW("analysisLog - JNI trace - Java-To-Native - Null or malformed object not dumped");
        return;
    }

    clazz = obj->clazz;
    ALOGD("analysisLog - JNI trace - Java-To-Native ----- Object dump address: %p (%s, %d bytes) -----",
        obj, clazz->descriptor, (int) clazz->objectSize);
    //printHexDump(obj, clazz->objectSize);
    //ALOGD("JNI trace - Fields:");
    while (clazz != NULL) {
        ALOGD("analysisLog - JNI trace - Java-To-Native -     class: %s", clazz->descriptor);
        for (i = 0; i < clazz->ifieldCount; i++) {
            const InstField* pField = &clazz->ifields[i];
            char type = pField->signature[0];
            ...
            if (type == 'F' || type == 'D') {
                ...
                ALOGD("analysisLog - JNI trace - Java-To-Native -         Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %d value = %.3f", i,
                    pField->name, pField->signature,
                    pField->accessFlags, pField->byteOffset, dval);
            } else {
                u8 lval;
                ...
                ALOGD("analysisLog - JNI trace - Java-To-Native -         Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %d value = 0x%08llx", i,
                    pField->name, pField->signature,
                    pField->accessFlags, pField->byteOffset, lval);
            }
        }
    ...
    if (dvmIsClassObject(obj)) {
        ...
        for (i = 0; i < ((ClassObject *)obj)->sfieldCount; ++i) {
            ...
            if (type == 'F' || type == 'D') {
                ...
                ALOGD("analysisLog - JNI trace - Java-To-Native -         Static Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %zd value = %.3f", i,
                     pField->name, pField->signature,
                     pField->accessFlags, byteOffset, dval);
            } else {
                u8 lval;
                ...
                ALOGD("analysisLog - JNI trace - Java-To-Native -         Static Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %zd value = 0x%08llx", i,
                     pField->name, pField->signature,
                     pField->accessFlags, byteOffset, lval);
            }
        }
...
/*
 * JNI trace debugging - Dump some information about a class.
 */
void JNIDumpClass(const ClassObject* clazz)
{
    int i;

    if (clazz == NULL) {
        ALOGW("analysisLog - JNI trace - Null or malformed object not dumped");
        return;
    }

    ALOGD("analysisLog - JNI trace ----- Object dump address: %p (%s, %d bytes) -----",
        clazz, clazz->descriptor, (int) clazz->objectSize);
    //printHexDump(obj, clazz->objectSize);
    //ALOGD("JNI trace - Fields:");

    ALOGD("analysisLog - JNI trace -     class: %s", clazz->descriptor);
    for (i = 0; i < clazz->ifieldCount; i++) {
        const InstField* pField = &clazz->ifields[i];
        char type = pField->signature[0];
        ...
        if (type == 'F' || type == 'D') {
            ...
            if (type == 'F')
                dval = dvmGetFieldFloat(clazz, pField->byteOffset);
            else
                dval = dvmGetFieldDouble(clazz, pField->byteOffset);

            ALOGD("analysisLog - JNI trace -         Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %d value = %.3f", i,
                pField->name, pField->signature,
                pField->accessFlags, pField->byteOffset, dval);
        } else {
            u8 lval;
            ...
            if (type == 'J')
                lval = dvmGetFieldLong(clazz, pField->byteOffset);
            else if (type == 'Z')
                lval = dvmGetFieldBoolean(clazz, pField->byteOffset);
            else
                lval = dvmGetFieldInt(clazz, pField->byteOffset);

            ALOGD("analysisLog - JNI trace -         Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %d value = 0x%08llx", i,
                pField->name, pField->signature,
                pField->accessFlags, pField->byteOffset, lval);
        }
    }

    const StaticField* sfields = &clazz->sfields[0];
    for (i = 0; i < clazz->sfieldCount; ++i) {
        ...
        if (type == 'F' || type == 'D') {
            ...
            ALOGD("analysisLog - JNI trace -         Static Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %zd value = %.3f", i,
                 pField->name, pField->signature,
                 pField->accessFlags, byteOffset, dval);
        } else {
            u8 lval;
            ...
            ALOGD("analysisLog - JNI trace -         Static Field[%d]: 'name = %s' 'type = %s' accessFlag = %04x byteOffset = %zd value = 0x%08llx", i,
                 pField->name, pField->signature,
                 pField->accessFlags, byteOffset, lval);
        }
    }
    ...
    for (i = 0; i < clazz->directMethodCount; i++) {
        const Method* dMethod = &clazz->directMethods[i];
        ALOGD("analysisLog - JNI trace -         dMethod[%d]: 'name = %s' 'type = %s' accessFlag = %04x, insns = %p, insSize = %d", i, dMethod->name, dMethod->shorty, dMethod->accessFlags, dMethod->insns, dMethod->insSize);
    }
    
    for (i = 0; i < clazz->virtualMethodCount; i++) {
        const Method* vMethod = &clazz->virtualMethods[i];
        ALOGD("analysisLog - JNI trace -         vMethod[%d]: 'name = %s' 'type = %s' accessFlag = %04x, insns = %p, insSize = %d", i, vMethod->name, vMethod->shorty, vMethod->accessFlags, vMethod->insns, vMethod->insSize);
    }
}
