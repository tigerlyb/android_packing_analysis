...
//=========================JNI trace instrumentation start=========================//

static void entry(const Method* method, const u4* args)
{
    char thisString[32] = { 0 };
    const u4* sp = args;
    if (!dvmIsStaticMethod(method)) {
    ...
    ALOGI("analysisLog - JNI trace - J2N - entry -> { class: %s - method: name = %s, signature = %s - args: (%s) }", className.c_str(), method->name, signature, argsString);
    free(signature);
}

static void exit(const Method* method, Thread* self, const JValue returnValue)
{
    std::string className(dvmHumanReadableDescriptor(method->clazz->descriptor));
    char* signature = dexProtoCopyMethodDescriptor(&method->prototype);
    if (dvmCheckException(self)) {
        Object* exception = dvmGetException(self);
        std::string exceptionClassName(dvmHumanReadableDescriptor(exception->clazz->descriptor));
        ALOGI("analysisLog - JNI trace - J2N - exit <- { class: %s - method: name = %s, signature = %s - threw: %s }", className.c_str(), method->name, signature, exceptionClassName.c_str());
    } else {
        char returnValueString[128] = { 0 };
        char returnType = method->shorty[0];
        appendValue(returnType, returnValue, returnValueString, sizeof(returnValueString), false);

        ALOGI("analysisLog - JNI trace - J2N - exit <- { class: %s - method: name = %s, signature = %s - returned: %s }", className.c_str(), method->name, signature, returnValueString);
    }

    free(signature);
}

static void dumpByteArray(const char* pidstring, const Method* method, ArrayObject* arrayObj, int isArg, int argIndex) {
    char bytearraypath[255] = {0};
    strcpy(bytearraypath, "/data/data/");
    strcat(bytearraypath, pkgname);
    strcat(bytearraypath, "/pid");
    strcat(bytearraypath, pidstring);
    strcat(bytearraypath, ".bytearray");

    char* data = (char*) (void*) arrayObj->contents;
    u4 length = arrayObj->length;

    int fd = open(bytearraypath, O_RDWR | O_APPEND | O_CREAT, 0644);
    if (fd > 0) {
        char prefix[255] = {0};
        if (isArg) {
            snprintf(prefix, sizeof(prefix), "class: %s, method: %s, arg[%d] length: %d, content: \n", method->clazz->descriptor, method->name, argIndex, length);
        } else {
            snprintf(prefix, sizeof(prefix), "class: %s, method: %s, return length: %d, content: \n", method->clazz->descriptor, method->name, length);
        }
...
static void dumpForceByteArray(const char* pidstring, const Method* method, ArrayObject* arrayObj, int isArg, int argIndex) {
    char bytearraypath[255] = {0};
    strcpy(bytearraypath, "/data/data/");
    strcat(bytearraypath, pkgname);
    strcat(bytearraypath, "/pid");
    ...
    if (fd > 0) {
        char prefix[255] = {0};
        if (isArg) {
            snprintf(prefix, sizeof(prefix), "class: %s, method: %s, force changed arg[%d] length: %d, content: \n", method->clazz->descriptor, method->name, argIndex, length);
        } else {
            snprintf(prefix, sizeof(prefix), "class: %s, method: %s, force changed return length: %d, content: \n", method->clazz->descriptor, method->name, length);
        }
        write(fd, prefix, strlen(prefix));
        write(fd, data, length);
        write(fd, "\n\n", 2);
...
bool isFromCustomizedLib(const Method* method) {

    int javaIncluded = strncmp(method->clazz->descriptor, "Ljava/", strlen("Ljava/"));
    int javaxIncluded = strncmp(method->clazz->descriptor, "Ljavax/", strlen("Ljavax/"));
    int androidIncluded = strncmp(method->clazz->descriptor, "Landroid/", strlen("Landroid/"));
    int comAndroidIncluded = strncmp(method->clazz->descriptor, "Lcom/android/", strlen("Lcom/android/"));
    int googleIncluded = strncmp(method->clazz->descriptor, "Lcom/google/", strlen("Lcom/google/"));
    int libcoreIncluded = strncmp(method->clazz->descriptor, "Llibcore/", strlen("Llibcore/"));
    int facebookIncluded = strncmp(method->clazz->descriptor, "Lcom/facebook/", strlen("Lcom/facebook/"));
    int dalvikIncluded = strncmp(method->clazz->descriptor, "Ldalvik/", strlen("Ldalvik/"));
    int harmonyIncluded = strncmp(method->clazz->descriptor, "Lorg/apache/harmony/", strlen("Lorg/apache/harmony/"));

    if (javaIncluded != 0 && javaxIncluded != 0 && androidIncluded != 0 && comAndroidIncluded != 0 && googleIncluded != 0 && libcoreIncluded != 0 && facebookIncluded != 0 && dalvikIncluded != 0 && harmonyIncluded != 0) {
        return true;
    }
...
void dvmCallJNIMethod(const u4* args, JValue* pResult, const Method* method, Thread* self) {
    ...
    int id = getuid();
    int pid = getpid();
    bool customizedLib = false;

    char pidstring[16] = {0};
    sprintf(pidstring, "%d", pid);
    
    if (pkg_id == id && strcmp(enable_jni_j2n_trace, "true") == 0) {

        customizedLib = isFromCustomizedLib(method);

        if (customizedLib) {
            /*
            const u1* insnsPtr = NULL;

            if ( (int) method->insns % 2 == 0) {
                insnsPtr = (const u1*) &method->insns[0];
            } else {
                insnsPtr = (const u1*) &method->insns[0] - 1;
            }
            
            const u2* insnsPtr2 = (const u2*) insnsPtr;
            const u4* insnsPtr4 = (const u4*) insnsPtr;
            */

            char* signature = dexProtoCopyMethodDescriptor(&method->prototype);

            ALOGI("analysisLog - JNI trace - J2N - { class: %s - method name: %s, signature: %s - native instruction address: %p }", method->clazz->descriptor, method->name, signature, method->insns);

            //ALOGI("analysisLog - JNI trace - J2N - { class: %s - method name: %s, signature: %s - native instructions 1-byte format: (%02x %02x %02x %02x %02x %02x %02x %02x) - native instruction address: %p }", method->clazz->descriptor, method->name, signature, insnsPtr[0], insnsPtr[1], insnsPtr[2], insnsPtr[3], insnsPtr[4], insnsPtr[5], insnsPtr[6], insnsPtr[7], method->insns);

            //ALOGI("analysisLog - JNI trace - J2N - { class: %s - method name: %s, signature: %s - native instructions 2-byte format: (%04x %04x %04x %04x %04x %04x %04x %04x) - native instruction address: %p }", method->clazz->descriptor, method->name, signature, insnsPtr2[0], insnsPtr2[1], insnsPtr2[2], insnsPtr2[3], insnsPtr2[4], insnsPtr2[5], insnsPtr2[6], insnsPtr2[7], method->insns);

            //ALOGI("analysisLog - JNI trace - J2N - { class: %s - method name: %s, signature: %s - native instructions 4-byte format: (%08x %08x %08x %08x %08x %08x %08x %08x) - native instruction address: %p }", method->clazz->descriptor, method->name, signature, insnsPtr4[0], insnsPtr4[1], insnsPtr4[2], insnsPtr4[3], insnsPtr4[4], insnsPtr4[5], insnsPtr4[6], insnsPtr4[7], method->insns);
        }
    }
        ...
        while (*shorty != '\0') {
            ...
            case 'L':
                ...
                if (modArgs[idx] != 0) {
                    ...
                    if (customizedLib) {
                        Object* argObject = dvmDecodeIndirectRef(self, (jobject) modArgs[idx]);
                        if (argObject != NULL) {
                            const char* argDescriptor = argObject->clazz->descriptor;
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s }", method->name, idx, argDescriptor);

                            if (strcmp(argDescriptor, "Ljava/lang/String;") == 0) {
                                StringObject* strObj = (StringObject*) argObject;
                                char* argValue = dvmCreateCstrFromString(strObj);
                                if (argValue != NULL) {
                                    int arg_length = strlen(argValue);
                                    ALOGI("analysisLog - JNI trace - J2N - { method: %s, String argLength: %d, String argValue: %s }", method->name, arg_length, argValue);

                                    /* force string arg value */
                                    if (j2n_function_arg_index[0] != 0) {
                                        if (strcmp(j2n_function_name, method->name) == 0 && idx == atoi(j2n_function_arg_index) && atoi(j2n_function_arg_length) == arg_length) {
                                            StringObject* force_arg_string = dvmCreateStringFromCstr(j2n_function_arg_value);
                                            modArgs[idx] = (u4) addLocalReference(self, (Object*) force_arg_string);
                                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, String argValue force change to: %s }", method->name, j2n_function_arg_value);
                                        } 
                                    }                                   
                                } else {
                                    ALOGI("analysisLog - JNI trace - J2N - { method: %s, String argValue: NULL }", method->name);
                                }
                            } 

                            if (strcmp(argDescriptor, "[B") == 0) {
                                ArrayObject* argObj = (ArrayObject*) argObject;
                                dumpByteArray(pidstring, method, argObj, 1, idx);

                                /* force byte array arg value */
                                if (strcmp(j2n_function_name, method->name) == 0 && idx == atoi(j2n_function_arg_index) && j2n_function_arg_length[0] != 0) {
                                    int byte_length = atoi(j2n_function_arg_length);
                                    char* byte_buffer = (char*) malloc(sizeof(char) * byte_length);

                                    int fbf = open("/data/forcebyte", O_RDONLY);
                                    if(fbf > 0) {
                                        read(fbf, byte_buffer, byte_length);

                                        ALOGI("analysisLog - JNI trace - J2N - { method: %s, ByteArray argValue force change to: %s, length: %d }", method->name, byte_buffer, byte_length);

                                        ArrayObject* forceByteArray = dvmAllocPrimitiveArray('B', byte_length, ALLOC_DEFAULT);
                                        
                                        if (forceByteArray != NULL) {
                                            memcpy(forceByteArray->contents, byte_buffer, byte_length);

                                            modArgs[idx] = (u4) addLocalReference(self, (Object*) forceByteArray);

                                            //ALOGI("dumpForceByteArray - { contents: %s, length: %d }", (char*) (void*) forceByteArray->contents, forceByteArray->length);

                                            dumpForceByteArray(pidstring, method, forceByteArray, 1, idx);
                                        }                                         
                                    }
                                    close(fbf);
                                    free(byte_buffer);
                            ...
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argObject: NULL }", method->name, idx);
                        }
                    }
            ...
            case 'D':
                //=========================JNI trace instrumentation start=========================//
                if (customizedLib && j2n_function_arg_index[0] != 0) {
                    if (strcmp(j2n_function_name, method->name) == 0 && idx == atoi(j2n_function_arg_index)) {
                        double d = atof(j2n_function_arg_value); 
                        //modArgs[idx] = (u4) d;
                        ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %f }", method->name, idx, shorty, d);
                    }
                }
                //=========================JNI trace instrumentation end=========================//
            case 'J':
                ...
                if (customizedLib && j2n_function_arg_index[0] != 0) {
                    ...
                    if (strcmp(j2n_function_name, method->name) == 0 && idx == atoi(j2n_function_arg_index)) {
                        s8 j = (s8) atoll(j2n_function_arg_value);
                        //modArgs[idx] = (u4) j;
                        ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %lld }", method->name, idx, shorty, j);
                    }
                }
                //=========================JNI trace instrumentation end=========================//
                idx++;
            ...
            default:
                ...
                if (customizedLib && j2n_function_arg_index[0] != 0) {
                    if (strcmp(j2n_function_name, method->name) == 0 && idx == atoi(j2n_function_arg_index)) {
                        if (strcmp(shorty, "Z")) {
                            if (strcmp(j2n_function_arg_value, "true") == 0) {
                                modArgs[idx] = 1;
                            } else if (strcmp(j2n_function_arg_value, "false") == 0) {
                                modArgs[idx] = 0;
                            } else {
                                ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, invalid boolean force value: %s }", method->name, idx, shorty, j2n_function_arg_value);
                                break;
                            }
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %s }", method->name, idx, shorty, modArgs[idx] ? "true": "false");                            
                        } else if (strcmp(shorty, "C")) {
                            modArgs[idx] = (u4) j2n_function_arg_value[0];
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %c }", method->name, idx, shorty, (char) modArgs[idx]);
                        } else if (strcmp(shorty, "F")) {
                            float f = (float) atof(j2n_function_arg_value); 
                            //modArgs[idx] = (u4) f;
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %f }", method->name, idx, shorty, f);
                        } else {
                            modArgs[idx] = (u4) atoi(j2n_function_arg_value);
                            ALOGI("analysisLog - JNI trace - J2N - { method: %s, argument[%d], argType: %s, value force change to: %d }", method->name, idx, shorty, modArgs[idx]);
    ...
    if (customizedLib) {
        ...
        if (strcmp(enable_lib_trace, "true") == 0) {
            if (j2n_function_name[0] == 0 || strcmp(j2n_function_name, method->name) == 0)
            libcTrace(method->name);
        }
    }
    ...
    if (customizedLib) {
        exit(method, self, *pResult);

        if (strcmp(enable_lib_trace, "true") == 0) {
            if (j2n_function_name[0] == 0 || strcmp(j2n_function_name, method->name) == 0)
            libcTraceStop(method->name);
        }
        
        char returnType = method->shorty[0];
        if (returnType == 'L' && pResult->l != NULL) {
            jobject objTemp = NULL;
            objTemp = (jobject) addLocalReference(self, (Object*) pResult->l);
            Object* returnObj = dvmDecodeIndirectRef(self, objTemp);

            if (returnObj != NULL) {
                //const char* returnDescriptor = returnObj->clazz->descriptor;
                //char* returnObjChar = strstr(returnDescriptor, "String"); 
                if (strcmp(returnObj->clazz->descriptor, "Ljava/lang/String;") == 0) {
                    StringObject* strReturnObj = (StringObject*) returnObj;
                    char* returnValue = dvmCreateCstrFromString(strReturnObj);
                    if (returnValue != NULL) {
                        ALOGI("analysisLog - JNI trace - J2N - { method: %s, String returnLength: %d, String returnValue: %s }", method->name, strlen(returnValue), returnValue);
                    } else {
                        ALOGI("analysisLog - JNI trace - J2N - { method: %s, String returnValue: NULL }", method->name);
                    }
                } 

                if (strcmp(returnObj->clazz->descriptor, "[B") == 0) {
                    ArrayObject* arrayReturnObj = (ArrayObject*) returnObj;
                    dumpByteArray(pidstring, method, arrayReturnObj, 0, 0);
                }
            } else {
                ALOGI("analysisLog - JNI trace - J2N - { returnObj: NULL }");
            }
        }

        if (strcmp(j2n_function_name, method->name) == 0 && j2n_function_return_value[0] != 0) {
            ALOGI("analysisLog - JNI trace - J2N - { method: %s, String returnLength: %d, String returnValue force change to: %s }", method->name, strlen(j2n_function_return_value), j2n_function_return_value);
            switch (method->shorty[0]) {
                case 'Z':
                    {
                        if (strcmp(j2n_function_return_value, "true") == 0) {
                            pResult->z = 1;
                        } 

                        if (strcmp(j2n_function_return_value, "false") == 0) {
                            pResult->z = 0;
                        } 
                    }
                    break;
                case 'B':
                    {
                        pResult->b = (s1) atoi(j2n_function_return_value);
                    }
                    break;
                case 'C':
                    {
                        pResult->c = (u2) j2n_function_return_value[0];
                    }
                    break;
                case 'S':
                    {
                        pResult->s = (s2) atoi(j2n_function_return_value);
                    }
                    break;
                case 'I':
                    {
                        pResult->i = (s4) atoi(j2n_function_return_value);
                    }
                    break;
                case 'J':
                    {
                        pResult->j = (s8) atoll(j2n_function_return_value);
                    }
                    break;
                case 'F':
                    {
                        pResult->f = (float) atof(j2n_function_return_value);
                    }
                    break;
                case 'D':
                    {
                        pResult->d = atof(j2n_function_return_value);
                    }
                    break;
                case 'L':
                    {
                        StringObject* force_return_string = dvmCreateStringFromCstr(j2n_function_return_value);
                        pResult->l = (Object* ) force_return_string;
                    }
                    break;
...
static void dumpStaticField (const StaticField* field)
{
    int id = getuid();
    if (pkg_id == id && strcmp(enable_jni_n2j_trace, "true") == 0) {
        const Method* thisMethod = dvmGetCurrentJNIMethod();

        bool customizedLib = isFromCustomizedLib(thisMethod);

        if (!customizedLib) {
            return;
        }

        char type = field->signature[0];
        switch (type) {
            case 'Z':
                {
                    bool fieldValueBool;
                    if (dvmIsVolatileField(field)) {
                        fieldValueBool = dvmGetStaticFieldBooleanVolatile(field);
                    } else {
                        fieldValueBool = dvmGetStaticFieldBoolean(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueBool ? "true" : "false", field->clazz->descriptor);
                }
                break;
            case 'B':
                {
                    s1 fieldValueByte;
                    if (dvmIsVolatileField(field)) {
                        fieldValueByte = dvmGetStaticFieldByteVolatile(field);
                    } else {
                        fieldValueByte = dvmGetStaticFieldByte(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %02x, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueByte, field->clazz->descriptor);
                }
                break;
            case 'S':
                {
                    s2 fieldValueShort;
                    if (dvmIsVolatileField(field)) {
                        fieldValueShort = dvmGetStaticFieldShortVolatile(field);
                    } else {
                        fieldValueShort = dvmGetStaticFieldShort(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %d, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueShort, field->clazz->descriptor);
                }
                break;
            case 'C':
                {
                    u2 fieldValueChar;
                    if (dvmIsVolatileField(field)) {
                        fieldValueChar = dvmGetStaticFieldCharVolatile(field);
                    } else {
                        fieldValueChar = dvmGetStaticFieldChar(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %c, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueChar, field->clazz->descriptor);
                }
                break;
            case 'I':
                {
                    s4 fieldValueInt;
                    if (dvmIsVolatileField(field)) {
                        fieldValueInt = dvmGetStaticFieldIntVolatile(field);
                    } else {
                        fieldValueInt = dvmGetStaticFieldInt(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %d, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueInt, field->clazz->descriptor);
                }
                break;
            case 'J':
                {
                    s8 fieldValueLong;
                    if (dvmIsVolatileField(field)) {
                        fieldValueLong = dvmGetStaticFieldLongVolatile(field);
                    } else {
                        fieldValueLong = dvmGetStaticFieldLong(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %lld, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueLong, field->clazz->descriptor);
                }
                break;
            case 'F':
                {
                    ...
                    if (dvmIsVolatileField(field)) {
                        fieldValueFloat = dvmGetStaticFieldFloatVolatile(field);
                    } else {
                        fieldValueFloat = dvmGetStaticFieldFloat(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %f, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueFloat, field->clazz->descriptor);
                }
                break;
            case 'D':
                {
                    ...
                    if (dvmIsVolatileField(field)) {
                        fieldValueDouble = dvmGetStaticFieldDoubleVolatile(field);
                    } else {
                        fieldValueDouble = dvmGetStaticFieldDouble(field);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, value: %f, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueDouble, field->clazz->descriptor);
                }
                break;
            case 'L':
            case '[':
                {
                    ...
                    if (dvmIsVolatileField(field)) {
                        fieldValueObject = dvmGetStaticFieldObjectVolatile(field);
                    } else {
                        fieldValueObject = dvmGetStaticFieldObject(field);
                    }
                    if (strcmp(field->signature, "Ljava/lang/String;") == 0) {
                        StringObject* fieldStringObject = (StringObject*) fieldValueObject;
                        char* stringValue = dvmCreateCstrFromString(fieldStringObject);
                        if (stringValue == NULL) {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, string value = NULL, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, field->clazz->descriptor);
                            break;
                        }
                        
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, stringLength: %d, string value: %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, strlen(stringValue), stringValue, field->clazz->descriptor);
                       
                    } else {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, type: %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, field->clazz->descriptor);
                    }
                }
                break;
            default:
                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get static field - { name: %s, signature = %s, invalid type %c, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, type, field->clazz->descriptor);
                break;
        }
    }
}

static void dumpField (const Object* obj, const InstField* field)
{
    int id = getuid();
    if (pkg_id == id && strcmp(enable_jni_n2j_trace, "true") == 0) {
        const Method* thisMethod = dvmGetCurrentJNIMethod();
            ...
            case 'Z':
                {
                    bool fieldValueBool;
                    if (dvmIsVolatileField(field)) {
                        fieldValueBool = dvmGetFieldBooleanVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueBool = dvmGetFieldBoolean(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueBool ? "true" : "false", field->clazz->descriptor);
                }
                break;
            case 'B':
                {
                    s1 fieldValueByte;
                    if (dvmIsVolatileField(field)) {
                        fieldValueByte = dvmGetFieldByteVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueByte = dvmGetFieldByte(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %02x, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueByte, field->clazz->descriptor);
                }
                break;
            case 'S':
                {
                    s2 fieldValueShort;
                    if (dvmIsVolatileField(field)) {
                        fieldValueShort = dvmGetFieldShortVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueShort = dvmGetFieldShort(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %d, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueShort, field->clazz->descriptor);
                }
                break;
            case 'C':
                {
                    u2 fieldValueChar;
                    if (dvmIsVolatileField(field)) {
                        fieldValueChar = dvmGetFieldCharVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueChar = dvmGetFieldChar(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %c, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueChar, field->clazz->descriptor);
                }
                break;
            case 'I':
                {
                    s4 fieldValueInt;
                    if (dvmIsVolatileField(field)) {
                        fieldValueInt = dvmGetFieldIntVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueInt = dvmGetFieldInt(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %d, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueInt, field->clazz->descriptor);
                }
                break;
            case 'J':
                {
                    s8 fieldValueLong;
                    if (dvmIsVolatileField(field)) {
                        fieldValueLong = dvmGetFieldLongVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueLong = dvmGetFieldLong(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %lld, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueLong, field->clazz->descriptor);
                }
                break;
            case 'F':
                {
                    float fieldValueFloat;
                    if (dvmIsVolatileField(field)) {
                        fieldValueFloat = dvmGetFieldFloatVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueFloat = dvmGetFieldFloat(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %f, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueFloat, field->clazz->descriptor);
                }
                break;
            case 'D':
                {
                    double fieldValueDouble;
                    if (dvmIsVolatileField(field)) {
                        fieldValueDouble = dvmGetFieldDoubleVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueDouble = dvmGetFieldDouble(obj, field->byteOffset);
                    }
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, value: %f, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, fieldValueDouble, field->clazz->descriptor);
                }
                break;
            case 'L':
            case '[':
                {
                    Object* fieldValueObject;
                    if (dvmIsVolatileField(field)) {
                        fieldValueObject = dvmGetFieldObjectVolatile(obj, field->byteOffset);
                    } else {
                        fieldValueObject = dvmGetFieldObject(obj, field->byteOffset);
                    }
                    if (strcmp(field->signature, "Ljava/lang/String;") == 0) {
                        StringObject* fieldStringObject = (StringObject*) fieldValueObject;
                        char* stringValue = dvmCreateCstrFromString(fieldStringObject);
                        if (stringValue == NULL) {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, string value = NULL, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, field->clazz->descriptor);
                            break;
                        }
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, stringLength: %d, string value = %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, strlen(stringValue), stringValue, field->clazz->descriptor);
                    } else {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, type: %s, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, field->clazz->descriptor);
                    }
                }
                break;
            default:
                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - Get field - { name: %s, signature = %s, invalid type %c, from class: %s }", thisMethod->name, thisMethod->insns, field->name, field->signature, type, field->clazz->descriptor);
                break;
        }
    }
}

static void dumpMethodV (Thread* self, const Method* method, JValue* pResult, va_list args)
{
    int id = getuid();
    if (pkg_id == id && strcmp(enable_jni_n2j_trace, "true") == 0) {
        const Method* thisMethod = dvmGetCurrentJNIMethod();
        ...
        char* signature = dexProtoCopyMethodDescriptor(&method->prototype);
        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p from %s - callMethodV - { name: %s, signature: %s, in class: %s }", thisMethod->name, thisMethod->insns, thisMethod->clazz->descriptor, method->name, signature, method->clazz->descriptor);  
        ...
        while (*desc != '\0') {
            switch (*desc++) {
                case 'Z':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: boolean, augValue: %s }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, u4) ? "true" : "false");
                    }
                    break;
                case 'B':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: byte, augValue: %02x }", thisMethod->name, thisMethod->insns, method->name, (u1) va_arg(args, u4));
                    }
                    break;

                case 'S':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: short, augValue: %d }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, u4));
                    }
                    break;
                case 'C':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: char, augValue: %c }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, u4));
                    }
                    break;

                case 'I':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: int, augValue: %d }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, u4));
                    }
                    break;
                case 'J':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: long, augValue: %lld }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, u8));
                    }
                    break;

                case 'F':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: float, augValue: %f }", thisMethod->name, thisMethod->insns, method->name, (float) va_arg(args, double));
                    }
                    break;
                case 'D':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: double, augValue: %f }", thisMethod->name, thisMethod->insns, method->name, va_arg(args, double));
                    }
                    break;
                case 'L': 
                case '[':                      
                    {
                        ...
                        Object* argObject = dvmDecodeIndirectRef(self, argObj);
                        if (argObject != NULL) {
                            const char* argDescriptor = argObject->clazz->descriptor;
                            ...
                            if (strcmp(argDescriptor, "Ljava/lang/String;") == 0) {
                                ...
                                char* argValue = dvmCreateCstrFromString(strObj);
                                if (argValue == NULL) {
                                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: %s, argValue: NULL }", thisMethod->name, thisMethod->insns, method->name, argDescriptor);
                                    break;
                                }

                                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: %s, argLength: %d, argValue: %s }", thisMethod->name, thisMethod->insns, method->name, argDescriptor, strlen(argValue), argValue);
                                
                            } else {
                                //ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, argType: %s }", thisMethod->name, thisMethod->insns, method->name, argDescriptor);
                                //JNIDumpObject(argObject);
                            }
                            
                        } else {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, arg: NULL }", thisMethod->name, thisMethod->insns, method->name);
                        }
                ...
                default:
                    ...
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, invalid argType: %c }", thisMethod->name, thisMethod->insns, method->name, *(desc-1));
                    break;
            }
        }
        ...
        switch (returnType) {
            case 'Z':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: boolean, returnValue: %s }", thisMethod->name, thisMethod->insns, method->name, pResult->z ? "true" : "false");
                }
                break;
            case 'B':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: byte, returnValue: %02x }", thisMethod->name, thisMethod->insns, method->name, pResult->b);
                }
                break;
            case 'S':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: short, returnValue: %d }", thisMethod->name, thisMethod->insns, method->name, pResult->s);
                }
                break;
            case 'C':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: char, returnValue: %c }", thisMethod->name, thisMethod->insns, method->name, pResult->c);
                }
                break;
            case 'I':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: int, returnValue: %d }", thisMethod->name, thisMethod->insns, method->name, pResult->i);
                }
                break;
            case 'J':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: long, returnValue: %lld }", thisMethod->name, thisMethod->insns, method->name, pResult->j);
                }
                break;
            case 'F':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: float, returnValue: %f }", thisMethod->name, thisMethod->insns, method->name, pResult->f);
                }
                break;
            case 'D':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: double, returnValue: %f }", thisMethod->name, thisMethod->insns, method->name, pResult->d);
                }
                break;
            case 'L':
            case '[':
                {   
                    ...
                    if (!dvmCheckException(self) && pResult->l != NULL) {

                        Object* returnObj = dvmDecodeIndirectRef(self, (jobject) pResult->l);
                        if (returnObj == NULL || returnObj->clazz == NULL) {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnObj: NULL }", thisMethod->name, thisMethod->insns, method->name);
                            break;
                        }
                        ...
                        if (strcmp(returnObj->clazz->descriptor, "Ljava/lang/String;") == 0) {
                            ...
                            if (returnValue == NULL) {
                                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: %s, returnStringValue: NULL }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor);
                                break;
                            }
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: %s, returnLength: %d, returnStringValue: %s }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor, strlen(returnValue), returnValue);
                        } else {

                            //ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: %s }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor);
                            //JNIDumpObject(returnObj);
                        }
                    } else {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnObj: NULL }", thisMethod->name, thisMethod->insns, method->name);
                    }
                ...
                break;
            default:
                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnType: %c }", thisMethod->name, thisMethod->insns, method->name, returnType);
                break;
        }


        if (strcmp(n2j_function_name, method->name) == 0 && n2j_function_return_value[0] != 0) {
            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodV - { name: %s, returnStringValue force change to: %s }", thisMethod->name, thisMethod->insns, method->name, n2j_function_return_value);
            switch (method->shorty[0]) {
                case 'Z':
                    {
                        if (strcmp(n2j_function_return_value, "true") == 0) {
                            pResult->z = 1;
                        } 

                        if (strcmp(n2j_function_return_value, "false") == 0) {
                            pResult->z = 0;
                        } 
                    }
                    break;
                case 'B':
                    {
                        pResult->b = (s1) atoi(n2j_function_return_value);
                    }
                    break;
                case 'C':
                    {
                        pResult->c = (u2) n2j_function_return_value[0];
                    }
                    break;
                case 'S':
                    {
                        pResult->s = (s2) atoi(n2j_function_return_value);
                    }
                    break;
                case 'I':
                    {
                        pResult->i = (s4) atoi(n2j_function_return_value);
                    }
                    break;
                case 'J':
                    {
                        pResult->j = (s8) atoll(n2j_function_return_value);
                    }
                    break;
                case 'F':
                    {
                        pResult->f = (float) atof(n2j_function_return_value);
                    }
                    break;
                case 'D':
                    {
                        pResult->d = atof(n2j_function_return_value);
                    }
                    break;
                case 'L':
                    {

                        StringObject* force_return_string = dvmCreateStringFromCstr(n2j_function_return_value);
                        pResult->l = (Object* ) force_return_string;
                    }
                    break;
...
static void dumpMethodA (Thread* self, const Method* method, JValue* pResult, const jvalue* args)
{
    int id = getuid();
    if (pkg_id == id && strcmp(enable_jni_n2j_trace, "true") == 0) {
        const Method* thisMethod = dvmGetCurrentJNIMethod();
        ...
        char* signature = dexProtoCopyMethodDescriptor(&method->prototype);
        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p from %s - callMethodA - { name: %s, signature: %s, in class: %s }", thisMethod->name, thisMethod->insns, thisMethod->clazz->descriptor, method->name, signature, method->clazz->descriptor);  
        ...
        while (*desc != '\0') {
            switch (*desc++) {
                case 'Z':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: boolean, augValue: %s }", thisMethod->name, thisMethod->insns, method->name, args->z ? "true" : "false");
                    }
                    break;
                case 'B':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: byte, augValue: %02x }", thisMethod->name, thisMethod->insns, method->name, args->b);
                    }
                    break;

                case 'S':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: short, augValue: %d }", thisMethod->name, thisMethod->insns, method->name, args->s);
                    }
                    break;
                case 'C':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: char, augValue: %c }", thisMethod->name, thisMethod->insns, method->name, args->c);
                    }
                    break;

                case 'I':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: int, augValue: %d }", thisMethod->name, thisMethod->insns, method->name, args->i);
                    }
                    break;
                case 'J':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: long, augValue: %lld }", thisMethod->name, thisMethod->insns, method->name, args->j);
                    }
                    break;

                case 'F':
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: float, augValue: %f }", thisMethod->name, thisMethod->insns, method->name, args->f);
                    }
                    break;
                case 'D':                       
                    {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: double, augValue: %f }", thisMethod->name, thisMethod->insns, method->name, args->d);
                    }
                    break;
                case 'L': 
                case '[':                      
                    {
                        Object* argObject = dvmDecodeIndirectRef(self, args->l);
                        if (argObject != NULL) {
                            const char* argDescriptor = argObject->clazz->descriptor;
                            ...
                            if (strcmp(argDescriptor, "Ljava/lang/String;") == 0) {
                                ...
                                if (argValue == NULL) {
                                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: %s, argValue: NULL }", thisMethod->name, thisMethod->insns, method->name, argDescriptor);
                                    break;
                                }

                                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: %s, argLength: %d, argValue: %s }", thisMethod->name, thisMethod->insns, method->name, argDescriptor, strlen(argValue), argValue);
                                
                            } else {
                                //ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, argType: %s }", thisMethod->name, thisMethod->insns, method->name, argDescriptor);
                                //JNIDumpObject(argObject);
                            }
                            
                        } else {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, arg: NULL }", thisMethod->name, thisMethod->insns, method->name);
                        }
                ...
                default:
                    ...
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, invalid argType: %c }", thisMethod->name, thisMethod->insns, method->name, *(desc-1));
                    break;
            }
        ...
        switch (returnType) {
            case 'Z':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: boolean, returnValue: %s }", thisMethod->name, thisMethod->insns, method->name, pResult->z ? "true" : "false");
                }
                break;
            case 'B':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: byte, returnValue: %02x }", thisMethod->name, thisMethod->insns, method->name, pResult->b);
                }
                break;
            case 'S':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: short, returnValue: %d }", thisMethod->name, thisMethod->insns, method->name, pResult->s);
                }
                break;
            case 'C':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: char, returnValue: %c }", thisMethod->name, thisMethod->insns, method->name, pResult->c);
                }
                break;
            case 'I':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: int, returnValue: %d }", thisMethod->name, thisMethod->insns, method->name, pResult->i);
                }
                break;
            case 'J':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: long, returnValue: %lld }", thisMethod->name, thisMethod->insns, method->name, pResult->j);
                }
                break;
            case 'F':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: float, returnValue: %f }", thisMethod->name, thisMethod->insns, method->name, pResult->f);
                }
                break;
            case 'D':
                {
                    ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: double, returnValue: %f }", thisMethod->name, thisMethod->insns, method->name, pResult->d);
                }
                break;
            case 'L':
            case '[':
                {
                    ...
                    if (!dvmCheckException(self) && pResult->l != NULL) {
                        ...
                        Object* returnObj = dvmDecodeIndirectRef(self, (jobject) pResult->l);
                        if (returnObj == NULL || returnObj->clazz == NULL) {
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnObj: NULL }", thisMethod->name, thisMethod->insns, method->name);
                            break;
                        }
                        ...
                        if (strcmp(returnObj->clazz->descriptor, "Ljava/lang/String;") == 0) {
                            ...
                            char* returnValue = dvmCreateCstrFromString(strReturnObj);
                            if (returnValue == NULL) {
                                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: %s, returnStringValue: NULL }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor);
                                break;
                            }
                            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: %s, returnLength: %d, returnStringValue: %s }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor, strlen(returnValue), returnValue);
                        } else {

                            //ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: %s }", thisMethod->name, thisMethod->insns, method->name, returnObj->clazz->descriptor);
                            //JNIDumpObject(returnObj);
                        }
                    } else {
                        ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnObj: NULL }", thisMethod->name, thisMethod->insns, method->name);
                ...
                break;
            default:
                ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnType: %c }", thisMethod->name, thisMethod->insns, method->name, returnType);
                break;
        }

        if (strcmp(n2j_function_name, method->name) == 0 && n2j_function_return_value[0] != 0) {
            ALOGI("analysisLog - JNI trace - N2J - CurrentJNIMethod: %s at %p - callMethodA - { name: %s, returnStringValue force change to: %s }", thisMethod->name, thisMethod->insns, method->name, n2j_function_return_value);
            switch (method->shorty[0]) {
                case 'Z':
                    {
                        if (strcmp(n2j_function_return_value, "true") == 0) {
...
#define GET_STATIC_TYPE_FIELD(_ctype, _jname, _isref)                       \
    ...
    {                                                                       \
        ...
        dumpStaticField(sfield); /* JNI trace */                        \
        return value;                                                       \
    }
...
#define CALL_VIRTUAL(_ctype, _jname, _retfail, _retok, _isref)              \
    ...
    {                                                                       \
        ...
        dumpMethodV(ts.self(), meth, &result, args); /* JNI trace */        \
        return _retok;                                                      \
    }                                                                       \
    ...
    {                                                                       \
        ...
        if (_isref && !dvmCheckException(ts.self()))                        \
            result.l = (Object*)addLocalReference(ts.self(), result.l);           \
        dumpMethodA(ts.self(), meth, &result, args); /* JNI trace */        \
        return _retok;                                                      \
    }
...
#define CALL_STATIC(_ctype, _jname, _retfail, _retok, _isref)               \
    ...
    {                                                                       \
        ...
        if (_isref && !dvmCheckException(ts.self()))                        \
            result.l = (Object*)addLocalReference(ts.self(), result.l);           \
        dumpMethodV(ts.self(), (Method*)methodID, &result, args); /* JNI trace */    \
        return _retok;                                                      \
    }                                                                       \
    ...
    {                                                                       \
        ...
        if (_isref && !dvmCheckException(ts.self()))                        \
            result.l = (Object*)addLocalReference(ts.self(), result.l);           \
        dumpMethodA(ts.self(), (Method*)methodID, &result, args); /* JNI trace */     \
        return _retok;                                                      \
    }
    ...
    const JNINativeMethod* methods, jint nMethods)
{
    ...
    //=========================JNI trace instrumentation start=========================//
    int id = getuid();
    if (pkg_id == id && clazz != NULL && strcmp(enable_jni_n2j_trace, "true") == 0) {
        for (int i = 0; i < nMethods; i++) {
            ALOGI("analysisLog - JNI trace - RegisterNativeMethods - { method name: %s, signature: %s, addr: %p, from class: %s at %p, JNIEnv at %p }", methods[i].name, methods[i].signature, methods[i].fnPtr, clazz->descriptor, clazz, env);
        }
    }
    //=========================JNI trace instrumentation end=========================//
