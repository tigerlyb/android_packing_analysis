...
//#undef LOG_NDEBUG
//#define LOG_NDEBUG 0 // enable debug
...
//=========================binder trace instrumentation start=========================//
static char bindertrace[10] = {0};
static bool b_readable = true;
static pthread_mutex_t b_read_mutex;
static bool b_flag = true;
static char pkgname[100] = {0};
static char writepath[255] = {0};

void* b_read_bindertrace(void* arg)
{
    FILE* fp = NULL;

    while (bindertrace[0] == 0) {
        fp = fopen("/data/bindertrace", "r");
        if (fp == NULL) {
            sleep(1);
            continue;
        }
        fgets(bindertrace, 9, fp);
        bindertrace[strlen(bindertrace) - 1] = 0;
        fgets(pkgname, 99, fp);
...
static const void* bt_printBinderTransactionData(TextOutput& out, const void* data)
{
    const binder_transaction_data* btd = (const binder_transaction_data*) data;
    if (btd->target.handle < 1024) {
        out << "target.desc=" << btd->target.handle << endl;
    } else {
        out << "target.ptr=" << btd->target.ptr << endl;
    }
    ...
    strcpy(writepath, "/data/data/");
    strcat(writepath, pkgname);
    strcat(writepath, "/binderdump");

    FILE* fp = NULL;
    fp = fopen(writepath, "awb");
    fwrite(btd->data.ptr.buffer, 1, btd->data_size, fp);
    fclose(fp);

    return btd+1;
}

static const void* bt_printReturnCommand(TextOutput& out, const void* _cmd)
{
    static const size_t N = sizeof(kReturnStrings) / sizeof(kReturnStrings[0]);
    const int32_t* cmd = (const int32_t*)_cmd;
    int32_t code = *cmd++;
    ...
    switch (code) {
        case BR_TRANSACTION:
        case BR_REPLY: {
            out << "analysisLog - Command: " << kReturnStrings[cmdIndex];
            out << ": " << indent;
            cmd = (const int32_t *) bt_printBinderTransactionData(out, cmd);
            out << dedent;
        } break;
...
static const void* bt_printCommand(TextOutput& out, const void* _cmd)
{
    static const size_t N = sizeof(kCommandStrings) / sizeof(kCommandStrings[0]);
    const int32_t* cmd = (const int32_t*)_cmd;
    int32_t code = *cmd++;
    ...
    switch (code) {
        case BC_TRANSACTION:
        case BC_REPLY: {
            out << "analysisLog - Command: " << kCommandStrings[cmdIndex];
            out << ": " << indent;
            cmd = (const int32_t *) bt_printBinderTransactionData(out, cmd);
            out << dedent;
        } break;
...
//=========================binder trace instrumentation end===========================//
...
status_t IPCThreadState::getAndExecuteCommand()
{
    ...
    if (uid) {
        if (b_readable) {
            pthread_mutex_lock(&b_read_mutex);
            if (b_readable) {
                b_readable = false;
                pthread_mutex_unlock(&b_read_mutex);
                pthread_t binder_thread;
                pthread_create(&binder_thread, NULL, b_read_bindertrace, NULL);
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    //=========================binder trace instrumentation start=========================//
    if (uid == atoi(bindertrace) && strcmp(bindertrace, "")) {
        TextOutput::Bundle _b(alog);
        if (outAvail != 0) {
            const void* cmds = (const void*) bwr.write_buffer;
            const void* end = ((const uint8_t*) cmds) + bwr.write_size;
            while (cmds < end) cmds = bt_printCommand(alog, cmds);
        }
    }
    //=========================binder trace instrumentation end===========================//
